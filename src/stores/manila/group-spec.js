...
export class GroupSpecStore extends Base {
  get client() {
    return client.manila.shareGroupTypes.groupSpecs;
  }
  ...
  getListDataFromResult = (result) => {
    const { group_specs } = result;
    const data = [];
    Object.keys(group_specs).forEach((key) => {
      data.push({
        id: key,
        keyName: key,
        name: key,
        value: group_specs[key],
      });
    });
    return data;
  };
